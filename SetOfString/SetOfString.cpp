...
public:
	...
	void addString(const string& s){
		...
		for(int i = 0; i < (int)s.size(); i++){
			...
			curVertex = getVertex(curVertex, s[i]);
...
Test generatorRandomTest(int length, int alphabet, int numberString, int numberQuery){
	Test test("Random string of length "+ toString(length) +
		" with alphabet from a to " + toString((char)alphabet + 'a') + ". " +
		"Number of strings equal to " + toString(numberString) + ". " +
		"Number of query equal to " + toString(numberQuery) + ".");
...
int main(){
	...
	Trie T;
	T.addString("abcaba");
	//cout << T.getNumberString("abacaba");
	//Test test = generatorRandomTest(10, 10, 100, 100);
	//compareSolutions({new StlSet(), new Trie()}, {0, 1}, test);
}
